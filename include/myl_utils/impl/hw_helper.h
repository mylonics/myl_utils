...
#define GPIO_INTERUPT_CB_HELPER(gpio, handler)                                              \
  ...
  void gpio##_cb_handler(const struct device *dev, struct gpio_callback *cb, uint32_t pins) \
  {                                                                                         \
    handler();                                                                              \
  }
...
#define INIT_GPIO_INTERUPT(gpio, edge) \
  INIT_GPIO_INTERUPT_WITH_HANDLER(gpio, gpio##_cb_handler, edge)
